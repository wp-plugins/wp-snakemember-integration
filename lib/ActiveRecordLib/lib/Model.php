...
class Model
{
	...
	public function __construct(array $attributes=array(), $guard_attributes=true, $instantiating_via_find=false, $new_record=true)
	{
        ...
        public static function merge(){
            ...
            elseif ( $numargs == 1){
                if (is_array($arg_list[0])){ // The Unique Array
                    return $arg_list[0];
                }
                else {
                    $ret = array(); // Make an Array
                    $ret[] = $arg_list[0];
                    return $ret;
                }
            }
                ...
                foreach ($merge_with as &$elem){
                    $discarded = false;
                    foreach ($ret as &$source_elem){
                        if ( $elem->id > 0 && ($elem->id == $source_elem->id )){
                            $discarded = true;
                            break;
                        }
                    }
                ...
                if ($numargs > 2){ //Recursive call
                    $class_name = get_called_class();
                    $ret = call_user_func_array( array($class_name, "merge") , array_merge($ret,array_slice($arg_list,2)));
                }
        ...
        public function get_ids_array($model_array){
            $ret = array();
            if ($model_array && is_array($model_array) && count($model_array)>0){
        ...
        public static function get_column_array($model_array,$column){
            $ret = array();
            if ($model_array && is_array($model_array) && count($model_array)>0){
                foreach ($model_array as &$m){
                    $ret[] = $m->$column;
